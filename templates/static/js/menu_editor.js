...
function remove_section(id) {
    sections[id] = '';
    items[id] = [];
    $('#section' + id).fadeOut(function () {
        $(this).remove();
    });
}
function remove_item(section_id, id) {
    items[section_id][id] = [];
    $('#section' + section_id + '-item' + id).fadeOut(function () {
        $(this).remove();
    });
}
...
function get_item_template(name, desc, price, item_id, section_id) {
    var template =
        ...
        '%name%' +
        '<input type="image" class="edit-delete-element" src="static/img/cross.png" width="25" height="25" onclick="remove_item(%section_id%,%item_id%)">' +
        '</div>' +
...
function update_section(id, new_name) {
    ...
    if (sections.indexOf(new_name) == -1
        ...
        $('#section' + id + '-edit-title').fadeOut(function () {
            $(this).remove();
...
function get_section_template(id) {
    var template =
        ...
        '%name%' +
        '</a>' +
        '<input type="image" class="edit-delete-element" src="static/img/cross.png" width="25" height="25" onclick="remove_section(%id%)">' +
        '<input type="image" class="edit-delete-element" src="static/img/edit.png" width="25" height="25" onclick="edit_section(%id%)">' +
        '</div>' +
